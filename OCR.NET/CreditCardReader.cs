...
namespace OCR.NET {
    public class CreditCardReader {
                ...
                foreach ( var box in boxes ) {
                    ...
                    if ( ar > MinNumberAspectRatio ) {
                        ...
                        using ( var ms = new MemoryStream() ) {
                            ...
                            if ( list.Count > 0 && list.FirstOrDefault().Confidence > Core.OCR.MIN_CONFIDENCE ) {
        ...
        protected List<OcrResult> TryProcess( Mat m, bool onlyNumbers = false ) {
            using ( var OCR = new Core.OCR() ) {
                int lv = 1;
                List<OcrResult> list = new List<OcrResult>();
                list.AddRange(OCR.GetResult(m, onlyNumbers));
                if ( list.FirstOrDefault(x => x.Confidence > Core.OCR.SAFE_CONFIDENCE) != null ) {
                    return list;
                }
                lv++; // 2
